	...
	: name(name), isSigned(false), signGrade(signGrade), executeGrade(executeGrade)
{
	...
	if (signGrade < 1 || executeGrade < 1)
		throw GradeTooHighException("Form grade is lower than the highest value\n");
	if (signGrade > 150 || executeGrade > 150)
		throw GradeTooLowException("Form grade is higher than the lowest value\n");
}
...
{
	return os << "Form " << form.getName() << ", is signed: " << (form.getIsSigned() ? "true" : "false")
